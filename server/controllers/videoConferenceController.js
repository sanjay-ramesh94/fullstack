...
const downloadReport = async (req, res) => {
  try {
    ...
    const bookings = await VideoConferenceBooking.find(matchQuery)
    ...
    res.setHeader('Content-Disposition', `attachment; filename="video-conference-report-${dateRange}-${new Date().toISOString().split('T')[0]}.pdf"`);
    ...
    doc.fontSize(18).font('Times-Bold')
       ...
       .text('PLACEMENT CELL - VIDEO CONFERENCING HALL', { align: 'center' })
    ...
    doc.fontSize(16).font('Times-Bold')
       ...
       .text(`Total Meetings: ${totalBookings}`)
    ...
    if (bookings.length === 0) {
      doc.fontSize(12).font('Times-Roman')
         .text('No meetings found for the selected criteria.');
    } else {
      // Table headers
      const tableTop = doc.y;
      const tableHeaders = ['Date', 'Time', 'Meeting', 'Organizer', 'Email', 'Phone', 'Dept', 'Type', 'Participants', 'Recording', 'Status'];
      const columnWidths = [50, 60, 75, 65, 80, 60, 40, 50, 55, 50, 45];
      const margin = 50;
      ...
      bookings.forEach((booking, index) => {
        ...
        const meetingName = booking.name || 'N/A';
        ...
        const meetingType = booking.meetingType || 'N/A';
        const participants = booking.expectedParticipants || 'N/A';
        const recording = booking.requiresRecording ? 'Yes' : 'No';
        const status = booking.status || 'N/A';

        // Truncate long text
        const safeMeetingName = meetingName.length > 11 ? meetingName.substring(0, 11) + '...' : meetingName;
        ...
        const safeMeetingType = meetingType.length > 7 ? meetingType.substring(0, 7) + '...' : meetingType;

        const rowData = [bookingDate, timeStr, safeMeetingName, safeOrganizer, safeEmail, safePhone, safeDept, safeMeetingType, participants, recording, status];
    ...
    console.error('❌ Error generating video conference report:', error);
