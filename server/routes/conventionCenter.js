...
router.get('/admin/test', auth, (req, res) => {
  res.json({ 
    message: 'Convention Center admin route is working',
    user: req.user,
    timestamp: new Date().toISOString()
  });
});

// Export Convention Center bookings
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const csvHeaders = [
        ...
        'End Time', 'Purpose', 'Expected Attendees', 'Status', 'Created At'
      ].join(',');
      ...
      res.attachment('convention-center-bookings.csv');
    ...
    console.error('Convention Center export error:', error);
