// src/components/user/ConventionCenterBookingForm.jsx
import React, { useState, useEffect } from 'react';
import { api } from '../../services/api';

const ConventionCenterBookingForm = ({ selectedDate, existingBookings, onBookingSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.department || !formData.purpose || 
        !formData.startTime || !formData.endTime || !formData.expectedAttendees) {
    ...
    try {
      ...
      await api.post('/convention-center', bookingData);
      onBookingSuccess();
    } catch (error) {
      setError(error.response?.data?.message || 'Failed to create convention center booking');
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-semibold text-gray-800 mb-6">Book Convention Center</h3>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                Confirm Convention Center Booking
              </>
            )}
          </button>
        </form>
...
export default ConventionCenterBookingForm;
